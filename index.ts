...
async function runMistral(message: string, extraSystemPrompt?: string) {
    ...
    const result = await mistral.chat.complete({
        ...
        messages: [
            { content: systemPrompt + extraSystemPrompt, role: "system" },
            {
                content: message,
                role: "user",
            },
...
app.post("/parse-email-mailgun", async (req, res) => {
    ...
    const parsingProfilesString = request["parsing-profiles"] as string;
    const parsingProfiles = JSON.parse(parsingProfilesString || "[]");
    console.log("Parsing profiles:", parsingProfiles);
    if (oldJob) {
    ...
    try {
        ...
        const json = await runMistral(textMinified, parsingProfilesString) as string;
        const parsedJson = JSON.parse(json);
        ...
        const response = {
            ...
            is_delivery: parsedJson?.is_delivery ?? false,
            identifier: parsedJson?.restaurant_name + " " + parsedJson?.pick_address.street_address,
            parsing_profile: parsedJson?.parsing_profile ?? null,
            job: parsedJson ?? json,
        }
        console.log("Final response:", response);

        await fs.writeFile(`${dirName}/ai-parsed.json`, JSON.stringify(response, null, 2));
        await fs.writeFile(`${dirName}/req.txt`, req.body);
        if (oldJob) {
